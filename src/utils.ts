...
const objectNameCache = new Map<any, string | null>();

// Clear cache utility
export const clearObjectNameCache = () => objectNameCache.clear();

export const _getObjectName = (target: any): string | null => {
  if (!target) return null;
  if (objectNameCache.has(target)) return objectNameCache.get(target) || null;

  try {
    if (target?.constructor?.name !== 'Object') {
      const name = target.constructor.name;
      objectNameCache.set(target, name);
      return name;
    }
    const debugName = getDebugName(target);
    const name = extractStoreName(debugName);
    objectNameCache.set(target, name);
    return name;
  } catch (error) {
    console.warn('Failed to get debug name:', error);
    objectNameCache.set(target, 'Unknown');
    return 'Unknown';
  }
};
...
export const formatToSequenceDiagram = events => {
  ...
  const participants = objects
    .map(objectName => `participant ${objectName}`)
    .join('\n');
